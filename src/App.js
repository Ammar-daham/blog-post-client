...
const App = () => {
  ...
  const createNewUser = async (newUser) => {
    ...
    if (res.type === 'users/newUser/rejected') {
      dispatch(setError(res.payload, 10))
    } else {
      dispatch(
        setSuccess(
          `Thank you ${res.payload.name}, welcome to our blog post app!`,
          10,
        ),
      )
    }
    ...
    <Router>
      <div>
        ...
        <Routes>
          ...
          <Route
            path="/sign-up"
            element={<SignupForm createUser={createNewUser} />}
          ></Route>
          <Route
            path="/login"
